            ...
            <div className="p-8 border-b border-white/10 flex flex-col md:flex-row justify-between items-center gap-6 bg-white/[0.01]">
              <div className="flex items-center gap-6">
                ...
                <div className="flex items-center gap-1 bg-black/40 p-1 rounded-xl border border-white/5 shadow-inner">
                  {(() => {
                    const startMonth = group?.startDate ? group.startDate.substring(0, 7) : (group?.createdAt ? group.createdAt.substring(0, 7) : '');
                    const isPrevDisabled = startMonth && selectedMonth <= startMonth;
                    return (
                      <button
                        onClick={isPrevDisabled ? undefined : handlePrevMonth}
                        disabled={isPrevDisabled}
                        className={`p-2 rounded-lg transition-all ${isPrevDisabled ? 'opacity-30 cursor-not-allowed text-gray-600' : 'hover:bg-white/5 text-gray-400 hover:text-white'}`}
                      >
                        <ChevronLeft size={16} />
                      </button>
                    );
      ...
      <Modal isOpen={isEnrollModalOpen} onClose={() => { setIsEnrollModalOpen(false); setSelectedStudents([]); }} title="Guruhga talaba qo'shish">
        <div className="space-y-5">
          <div>
            ...
            <input type="date" min={group?.startDate || group?.createdAt?.substring(0, 10) || ''} value={enrollDate} onChange={e => setEnrollDate(e.target.value)} className="w-full bg-[#131520] border border-white/10 rounded-xl px-4 py-3.5 text-white focus:ring-2 focus:ring-indigo-500 outline-none" required />
          </div>
          <div className="relative">

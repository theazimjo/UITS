...
import {
  LayoutDashboard, ClipboardCheck, BookOpen, BarChart3, Settings, LogOut, GraduationCap, FileText, Award
} from 'lucide-react';
...
const TeacherSidebar = ({ onLogout }) => {
  const navItems = [
    ...
    { to: '/teacher/certificates', icon: <Award size={18} />, label: 'Sertifikat' },
  ...
  return (
    <aside className="w-[260px] bg-white/30 dark:bg-black/20 flex flex-col py-4 px-3 z-20 flex-shrink-0 backdrop-blur-md hidden md:flex">
      {/* Brand Header */}
      <div className="flex items-center gap-3 mb-8 px-3 mt-2 select-none">
        <div className="w-8 h-8 bg-gradient-to-tr from-emerald-500 to-teal-600 rounded-[10px] flex items-center justify-center shadow-sm">
          <GraduationCap className="text-white w-5 h-5" strokeWidth={1.5} />
        </div>
        <span className="text-lg font-semibold text-black dark:text-white tracking-tight">
          UITS <span className="text-emerald-600 dark:text-emerald-400 font-medium">Teacher</span>
        </span>
      </div>

      {/* Navigation */}
      <div className="px-1 text-[11px] font-semibold text-gray-500 dark:text-gray-400 tracking-wider uppercase mb-2">O'qituvchi paneli</div>
      <nav className="flex-1 flex flex-col gap-0.5">
        {navItems.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                <div className={`${isActive ? 'text-white' : 'text-emerald-500 dark:text-emerald-400 opacity-80'}`}>
                  {item.icon}
                </div>
